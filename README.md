...
Дополнительно для `apply-vacancies` можно использовать переменные окружения:

| Переменная окружения      | Описание                                                                                  |
| ------------------------- | ----------------------------------------------------------------------------------------- |
| `HH_APPLY_FORCE_MESSAGE`  | Форсирует отправку сопроводительного письма (`true/false`, `1/0`, `yes/no`, `on/off`)   |
| `HH_APPLY_COVER_LETTER`   | Текст сопроводительного письма (поддерживает шаблоны и плейсхолдеры как в `--letter`)   |
| `HH_APPLY_EXCLUDED_KEYWORDS` | Исключает вакансии по ключевым словам в названии вакансии **и названии компании-работодателя**; разделители: запятая, `;` или новая строка |

### Использование .env файла

Для удобства настройки можно создать файл `.env` в корне проекта и загрузить его:

```bash
# Загрузка переменных окружения из .env перед запуском
...
**Вариант 2: напрямую в environment**
```yaml
services:
  hh_applicant_tool:
    environment:
      - HH_APPLY_FORCE_MESSAGE=true
      - HH_APPLY_COVER_LETTER=Приветствую! Готов к интервью.
```

> [!IMPORTANT]
> При использовании плейсхолдеров в `HH_APPLY_COVER_LETTER` убедитесь, что используете только доступные:
> - `%(vacancy_name)s` - название вакансии
> - `%(employer_name)s` - название работодателя
...
> - `%(resume_url)s` - ссылка на резюме
>
> Если плейсхолдер не может быть подставлен (например, отсутствует в данных), будет выведено предупреждение в логах и будет использован исходный текст без подстановки.
