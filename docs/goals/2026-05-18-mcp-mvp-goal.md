...
- 2026-05-19 00:12: Re-tried a protocol-level `mcp.client.stdio` initialize/list-tools smoke with a 10s `anyio.fail_after`; it still hung in this environment without producing useful output. Do not use that attempted check as completion evidence. Current stdio evidence remains subprocess startup/no stdout contamination, console-script help, all eight tools registered, and in-process FastMCP `hh_whoami` call.
...
- Real HH `/negotiations` integration is covered with fake API tests; live HH apply cannot be integration-tested without credentials and must remain guarded by `--allow-apply`, `dry_run=false`, and `confirm_apply=true`.
