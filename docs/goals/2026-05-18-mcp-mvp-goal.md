...
- 2026-05-19 00:12: Closed MCP error/audit safety gaps found during acceptance audit. Tool exceptions after `mcp_runs` creation now mark the active run `failed` with `error_count=1` and still flush token/cookies. Error envelopes now map HH API/network classes to concrete codes (`auth_expired`, `forbidden`, `not_found`, `rate_limited`, `manual_form_required`, `upstream_error`, `timeout`, `network_error`). Real apply safety now checks current `vacancy_response_dedup` even when the request supplies a stale `analysis_id`, so legacy/current dedupe cannot be bypassed. Added tests for failed run audit, error mapping, error-path auth flush, and stale-analysis dedupe block. Verified `PYTHONPATH=/tmp/codex-poetry python -m poetry run pytest` (`91 passed`), `PYTHONPATH=/tmp/codex-poetry python -m poetry run ruff check .`, `PYLINTHOME=/tmp/pylint PYTHONPATH=/tmp/codex-poetry python -m poetry run pylint hh_llm_agent/ src/hh_applicant_tool/`, `python -m build`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-mcp --help`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-tool --help`, and `git diff --check`.
- 2026-05-19 00:33: Closed the last stdio acceptance gap. Replaced the MCP startup path with a repo-local async stdio bridge that feeds `FastMCP._mcp_server.run(...)` from `sys.stdin.buffer.readline()` and writes only JSON-RPC frames to `stdout`. Extended `tests/test_mcp_stdio.py` from a startup smoke into a protocol-level subprocess check: `initialize`, `notifications/initialized`, then `tools/list`, asserting all eight MVP tool names. Verified `PYTHONPATH=/tmp/codex-poetry python -m poetry run pytest tests/test_mcp_stdio.py tests/test_mcp_tools.py -q` (`10 passed`), `python -m compileall -q src/hh_applicant_tool/mcp/server.py tests/test_mcp_stdio.py`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run pytest` (`92 passed`), `PYTHONPATH=/tmp/codex-poetry python -m poetry run ruff check .`, `PYLINTHOME=/tmp/pylint PYTHONPATH=/tmp/codex-poetry python -m poetry run pylint hh_llm_agent/ src/hh_applicant_tool/`, `python -m build`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-mcp --help`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-tool --help`, and `git diff --check`.

## Risks and Blockers

- Real HH `/negotiations` integration is covered with fake API tests; live HH apply cannot be integration-tested without credentials and remains guarded by `--allow-apply`, `dry_run=false`, and `confirm_apply=true`.
- Existing CLI behavior stayed additive in this goal, but future MCP changes should keep validating both the CLI and subprocess stdio protocol path.

## Final Verification

- Programmatic profile context is in place: `HHProfileContext` lives in `src/hh_applicant_tool/context.py`, `HHApplicantTool.from_profile(...)` is available in `src/hh_applicant_tool/main.py`, and `tests/test_context.py` covers profile resolution, proxy/session separation, user-agent persistence, and the compatibility shim.
- `VacancyResearchService` is the MCP-facing service layer in `src/hh_applicant_tool/services/vacancy_research.py`; it handles search, detail fetch, structured analysis, cover letters, dry-run planning, real apply, and fail-closed safety checks without reusing CLI operations.
- MCP audit persistence is implemented in `src/hh_applicant_tool/storage/queries/schema.sql`, `src/hh_applicant_tool/storage/facade.py`, and matching models/repositories for `mcp_runs`, `vacancy_analysis`, and `application_attempts`; `tests/test_vacancy_audit_storage.py` and `tests/test_vacancy_research_service.py` prove dry-run audit, applied-count limits, and blocking attempts.
- The stdio MCP server is live at `src/hh_applicant_tool/mcp/server.py` and `hh-applicant-mcp`; `tests/test_mcp_stdio.py` now proves subprocess `initialize` plus `tools/list`, confirms clean stdout before protocol input, and asserts all eight MVP tools: `hh_whoami`, `hh_list_resumes`, `hh_search_vacancies`, `hh_get_vacancy`, `hh_analyze_vacancy`, `hh_research_vacancies`, `hh_apply_vacancy`, `hh_research_and_apply`.
- Destructive flows remain fail-closed: `src/hh_applicant_tool/mcp/context.py` defaults `allow_apply` to `False`; `src/hh_applicant_tool/mcp/tools.py` defaults `dry_run=True`; `src/hh_applicant_tool/services/vacancy_research.py` requires server `allow_apply`, request `dry_run=false`, and `confirm_apply=true`, blocks archived/manual-form/test/relation/dedupe/prior-attempt cases, keeps dry-run out of `vacancy_response_dedup`, and writes dedupe only after successful real apply.
- README MCP docs are present in `README.md`, including startup commands, config shape, policy, tool list, audit tables, and destructive safety gates.
- Validation contract passed on the current tree: `PYTHONPATH=/tmp/codex-poetry python -m poetry run pytest` (`92 passed`), `PYTHONPATH=/tmp/codex-poetry python -m poetry run ruff check .`, `PYLINTHOME=/tmp/pylint PYTHONPATH=/tmp/codex-poetry python -m poetry run pylint hh_llm_agent/ src/hh_applicant_tool/` (`10.00/10`), `python -m build`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-mcp --help`, `PYTHONPATH=/tmp/codex-poetry python -m poetry run hh-applicant-tool --help`, and `git diff --check`.
- Completion audit result: the goal objective from `docs/PRD.md` is satisfied end to end for the MCP MVP scope, with repo-local docs, tests, README coverage, and no broken existing CLI checks in the verified suite.
